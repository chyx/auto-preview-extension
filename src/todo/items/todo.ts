...
class Todo extends Item {
  ...
  /* STATUS */
  makeStatus(state: string): ToDoStatus {
  ...
  setStatus(is: ToDoStatus, was = this.getStatus()) {
    if (_.isEqual(is, was)) {
      return;
    }
  ...
  finish(isPositive?: boolean) {
    ...
    if (
      ...
      Consts.symbols.box ===
        (isPositive ? Consts.symbols.done : Consts.symbols.cancelled)
    ) {
      this.unfinish();
      ...
      if (Config.getKey("timekeeping.elapsed.enabled") && started) {
        const startedFormat = Config.getKey("timekeeping.started.format"),
          ...
          time = Time.diff(new Date(), startedDate, elapsedFormat),
  ...
  setSymbol(symbol: string) {
    ...
    this.lineNextText = `${this.lineNextText.substring(0, startIndex)}${
      symbol ? `${symbol} ` : ""
    }${this.lineNextText.substring(endIndex)}`;
  }
